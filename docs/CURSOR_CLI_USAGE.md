...
- **单轮**：Broker 为当前轮生成/写入 `works/{{task_name}}-{{run_id}}-{{role}}/task.json`（含 `objective`、`instructions`、`mode` 等）→ 启动 Agent 容器（传 `WORK_DIR_REL`）→ Agent 内执行一次 `cursor run -p --force "…"` → 容器退出，写出该子目录下的 `result.json` 和日志。
- **多轮**：Broker 将「指定目标」拆成多步（例如 JSON 中的 `steps`）。每一轮：
  1. Broker 根据当前步生成本轮的 `task.json`（可包含上轮结果摘要、本步目标、约束）。
  2. Broker 在 workspace 下写入 `works/{{task_name}}-{{run_id}}-{{role}}/task.json`，然后启动 Agent 容器。
...
Broker: 目标 + steps，获取 run_id（snowflake_id 或本地生成）
  → 第 1 轮: 写 works/{{task_name}}-{{run_id}}-{{role}}/task.json (objective=step1, ...)
...
- `task_name` 来自任务 JSON 的 `task.id`；`run_id` 为本次执行的 id（snowflake_id：可通过环境变量 `SNOWFLAKE_ID_API_URL` 调用接口获得，或 v0.1 本地生成）；`role` 为当前 agent 的 role。
- Broker 启动容器时设置环境变量 `WORK_DIR_REL=works/{{task_name}}-{{run_id}}-{{role}}`，Agent 从该子目录读写 task.json、result.json、agent.log。
