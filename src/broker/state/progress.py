...
def _save_progress_dict(task_id: str, progress_dict: dict[str, list[dict]]) -> None:
    """Write progress dict to progress file."""
...
def _save_all_runs(task_id: str, runs: list[dict], subtask_id: str | None = None) -> None:
    """Write all run records for a subtask_id under task_id."""
    key = subtask_id or task_id
    progress_dict = _load_progress_dict(task_id)
    progress_dict[key] = runs
    _save_progress_dict(task_id, progress_dict)
...
def save_progress(
    ...
    Most recent run is at list end."""
    runs = _load_all_runs(task_id, subtask_id)
    payload = {
        "run_id": run_id,
        "task_id": task_id,
    ...
    _save_all_runs(task_id, runs, subtask_id)
...
def clear_progress(
    ...
    """Remove progress based on parameters.
    ...
    _save_progress_or_cleanup(progress_dict, task_id)
...
def _save_progress_or_cleanup(progress_dict, task_id):
    if progress_dict:
        _save_progress_dict(task_id, progress_dict)
    else:
        path = _progress_file(task_id)
