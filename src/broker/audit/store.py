"""
Persist audit records under .state/workers/<task_id>/audit/.
...
def _audit_dir(task_id: str) -> Path:
    return PROJECT_ROOT / ".state" / "workers" / task_id / "audit"


def _audit_file(task_id: str) -> Path:
    return _audit_dir(task_id) / "audit.json"


def load_audits(task_id: str) -> list[dict]:
    """Load all audit records for task_id; returns list, newest last."""
    path = _audit_file(task_id)
    if not path.exists():
        return []
    try:
...
def save_audit_record(task_id: str, record: dict) -> None:
    ...
    records = load_audits(task_id)
    records.append(record)
    _audit_file(task_id).write_text(
        json.dumps(records, indent=2, ensure_ascii=False)
    )


def list_task_ids_with_audits() -> list[str]:
    """List task_ids that have at least one audit record under .state/workers/<task_id>/audit/."""
...
def get_audit_summary_for_boost(
    exclude_task_id: str = "boost",
    max_per_task: int = 10,
) -> str:
    """
    Build a short summary of audit findings from other workers for injection into boost.
    Phase 5.1: audit-to-bootstrap. Only includes workers other than exclude_task_id.
    Prefer escalated or non-pass records; cap at max_per_task records per task.
    """
    task_ids = [tid for tid in list_task_ids_with_audits() if tid != exclude_task_id]
    if not task_ids:
        return ""

    lines: list[str] = ["Audit context from other workers (consider for system fixes):"]
    for tid in task_ids:
        records = load_audits(tid)
        # newest last; take last N
        recent = records[-max_per_task:] if len(records) > max_per_task else records
        ...
        for r in relevant:
            ...
            parts = [f"task {tid} round {round_i}: {conclusion}"]
            if criteria:
